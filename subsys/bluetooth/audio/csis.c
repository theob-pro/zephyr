...
LOG_MODULE_REGISTER(bt_csis, CONFIG_BT_CSIS_LOG_LEVEL);
			     ...
			     void *buf, uint16_t len, uint16_t offset)
{
	...
	if (csis->srv.cb != NULL && csis->srv.cb->sirk_read_req != NULL) {
		...
		if (cb_rsp == BT_CSIS_READ_SIRK_REQ_RSP_ACCEPT) {
			...
			err = sirk_encrypt(conn, &csis->srv.set_sirk,
					   &enc_sirk);
			if (err != 0) {
				LOG_ERR("Could not encrypt SIRK: %d", err);
	...
	LOG_DBG("Set sirk %sencrypted", sirk->type == BT_CSIS_SIRK_TYPE_PLAIN ? "not " : "");
