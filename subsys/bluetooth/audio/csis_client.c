...
LOG_MODULE_REGISTER(bt_csis_client, CONFIG_BT_CSIS_CLIENT_LOG_LEVEL);
				...
				const void *data, uint16_t length)
{
	...
	if (csis_inst != NULL) {
		if (length == sizeof(csis_inst->cli.set_lock)) {
			...
			LOG_DBG("Instance %u lock was %s", csis_inst->cli.idx,
			     ...
			     struct bt_gatt_discover_params *params)
{
	...
	if (attr == NULL) {
		LOG_DBG("Setup complete for %u / %u", cur_inst->cli.idx + 1, client->inst_count);
	...
	if (err != 0) {
		...
		if (length == 1) {
			(void)memcpy(&client->csis_insts[cur_inst->cli.idx].cli.rank,
				     data, length);
			LOG_DBG("%u", client->csis_insts[cur_inst->cli.idx].cli.rank);
		} else {
