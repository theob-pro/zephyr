...
LOG_MODULE_REGISTER(bt_mesh_cfg_srv, LOG_LEVEL);
		       ...
		       struct net_buf_simple *buf)
{
	...
	LOG_DBG("elem_addr 0x%04x pub_addr 0x%04x cred_flag %u",
	       elem_addr, pub_addr, cred_flag);
	LOG_DBG("pub_app_idx 0x%03x, pub_ttl %u pub_period 0x%02x",
			   ...
			   struct net_buf_simple *buf)
{
	...
	LOG_DBG("net_idx 0x%04x app_idx 0x%04x src 0x%04x lpn_addr 0x%02x",
			     ...
			     struct net_buf_simple *buf)
{
	...
	LOG_DBG("sub_src 0x%04x sub_dst 0x%04x period 0x%02x",
