...
TO_DELETE="${simulation_id}_client.log ${simulation_id}_bad_client.log ${simulation_id}_server.log"
...
if [ "${1}" != 'debug1' ]; then
  Execute "./${test_exe}" \
    -v=${verbosity_level} -s=${simulation_id} -d=1 -testid=bad_central -argstest "bad_client"
fi

if [ "${1}" != 'debug2' ]; then
  Execute "./${test_exe}" \
    -v=${verbosity_level} -s=${simulation_id} -d=2 -testid=peripheral -argstest "server"
fi
