...
static void subscribe(void)
{
	...
	ASSERT(!err, "bt_gatt_subscribe failed (%d)\n", err);
			     ...
			     struct bt_gatt_discover_params *params)
{
	...
	if (attr == NULL) {
		for (int i = 0; i < ARRAY_SIZE(gatt_handles); i++) {
			...
			ASSERT(gatt_handles[i] != 0, "did not find all handles\n");
		}
	...
	if (params->type == BT_GATT_DISCOVER_CHARACTERISTIC) {
		...
		const struct bt_uuid_16 ccc_uuid = BT_UUID_INIT_16(BT_UUID_GATT_CCC_VAL);
		...
		if (bt_uuid_cmp(chrc->uuid, BT_UUID_GATT_SC) == 0) {
			...
			ASSERT(!err, "bt_gatt_discover failed (%d)\n", err);

			return BT_GATT_ITER_STOP;
		}

	} else if (params->type == BT_GATT_DISCOVER_DESCRIPTOR && bt_uuid_cmp(params->uuid, BT_UUID_GATT_CCC) == 0) {
		LOG_DBG("found ccc");
